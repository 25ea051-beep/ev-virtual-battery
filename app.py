...
app.py  — v2
============
Flask backend for EV Virtual Battery Digital Twin.
Includes equal-weight CAN model, OOD detection, and sensitivity endpoints.
"""
...
from virtual_battery import (VirtualBattery, DRIVE_CYCLES, get_cell_ocv, SOHPredictor)
from virtual_battery_model import (
    VirtualBatteryCANModel, get_or_train_model,
    DEFAULT_DATASET_PATH, INPUT_META, TRAINING_RANGES
)
...
class TwinManager:
    def __init__(self):
        ...
        try:
            ...
            print(f"[TwinManager] CAN model load failed: {e}")
            self.can_model = None
        self.twin = None
    ...
    def set_config(self, age_years=None, cycle=None, ambient_temp=None,
                   dt=None, initial_soc=None, manual_current=False):
        changed = False
        if age_years is not None and age_years != self.age_years:
            self.age_years = float(age_years); changed = True
        if cycle is not None and cycle in DRIVE_CYCLES and cycle != self.cycle:
            self.cycle = str(cycle); changed = True
        if ambient_temp is not None and ambient_temp != self.ambient_temp:
            self.ambient_temp = float(ambient_temp); changed = True
        if dt is not None and dt != self.dt:
            self.dt = float(dt); changed = True
        if initial_soc is not None:
            soc_val = float(initial_soc)
            if soc_val > 1.0: soc_val /= 100.0
            if abs(soc_val - self.initial_soc) > 1e-4:
                self.initial_soc = soc_val; changed = True
    ...
    def tick(self, current=None):
        with self._lock:
            ...
            prev_len    = len(self.twin.alerts)
            frame       = self.twin.tick(current=eff_current)
            new_alerts  = self.twin.alerts[prev_len:]
...
def api_tick():
    ...
    return jsonify(manager.tick(current=current))
...
def api_model_info():
    ...
    global_imp = {}
    if manager.can_model and manager.can_model.is_trained:
        global_imp = manager.can_model.get_global_importances()

    samples = [
        {"id": "EVB_0010", "name": "EVB_0010 — Healthy Pack",
         "cycle_count": 171,  "voltage": 371.93, "current": 63.45,
         "temp_cell": 23.5,   "soc": 88.53,
         "expected_soh": 97.03, "expected_cond": "Healthy"},
        {"id": "EVB_0003", "name": "EVB_0003 — Moderate Aging",
         "cycle_count": 910,  "voltage": 378.93, "current": 61.32,
         "temp_cell": 33.0,   "soc": 93.19,
         "expected_soh": 82.32, "expected_cond": "Moderate"},
        {"id": "EVB_0001", "name": "EVB_0001 — Degraded Pack",
         "cycle_count": 1176, "voltage": 324.25, "current": 76.06,
         "temp_cell": 17.81,  "soc": 20.55,
         "expected_soh": 75.55, "expected_cond": "Degraded"},
        {"id": "EVB_0007", "name": "EVB_0007 — Critical End-of-Life",
         "cycle_count": 1774, "voltage": 377.84, "current": 31.52,
         "temp_cell": 48.74,  "soc": 99.28,
         "expected_soh": 62.87, "expected_cond": "Critical"},
    ]
    return jsonify({
        "is_trained":        bool(manager.can_model and manager.can_model.is_trained),
        "metrics":           metrics,
        "feature_cols":      list(getattr(manager.can_model, "FEATURE_COLS", [])),
...
def api_sensitivity():
    """
    Returns local feature sensitivity for the given CAN inputs.
    Used by the UI radar chart to show each input's actual influence on the prediction.
    """
    data = request.get_json() or {}
    try:
        model = manager.can_model or get_or_train_model()
        ...
        return jsonify({
            "status":            "success",
            "feature_influence": result["feature_influence"],
            "ood_detection":     result["ood_detection"],
        })
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 400
...
def main():
    port = 5000
    print("=" * 70)
    print("  EV VIRTUAL BATTERY DIGITAL TWIN — Command Center v2")
