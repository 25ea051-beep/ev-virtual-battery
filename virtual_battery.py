...
#    Delegated to virtual_battery_model.py (v2 equal-weight model)
# ─────────────────────────────────────────────────────────────────────────────
# Import the v2 model class so that joblib deserialization always resolves
# to the up-to-date VirtualBatteryCANModel (with OOD detection, physics fallback,
# feature engineering, and local sensitivity computation).
try:
    from virtual_battery_model import (
        VirtualBatteryCANModel,
        get_or_train_model,
        DEFAULT_DATASET_PATH as _VB_MODEL_DATASET_PATH,
        DEFAULT_MODEL_PATH   as _VB_MODEL_MODEL_PATH,
    )
except ImportError:
    # Fallback stub so rest of module still loads during testing
    class VirtualBatteryCANModel:  # type: ignore
        is_trained = False
        FEATURE_COLS = []
        metrics = {}
        ...
        def generate_virtual_battery(self, *a, **kw): return {}
    def get_or_train_model(*a, **kw):  # type: ignore
        raise RuntimeError("virtual_battery_model not available")
